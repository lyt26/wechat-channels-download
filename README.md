...
**Q：提示 WinError 10054 / 远程主机强迫关闭连接？**  
这是网络被中途掐断（解析服务器或视频 CDN 偶发断开），不是链接坏了。从 v1.2.4 起会：自动多次重试、默认绕过可能有问题的系统代理、失败时改用系统 `curl`。界面里再点一次「开始下载」通常就能成功。若仍失败：关掉 VPN/代理，或换网络再试。
